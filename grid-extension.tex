\chapter{Extending Polyhedral DGGS's to Create 3D Prismatoid Grids} \label{chap:extension}
While our modifications of SDOG refinement result in grids with many desirable properties, they may not be the best choice for all applications.
Due to widely differing needs, no single DGGS---2D or 3D---is ideal for all situations.
This need to have different grids for different applications is one reason there is such a wide variety of DGGS's currently available.
...
In this chapter we present method for creating 3D DGGS that uses conventional one---input DGGS---as starting point and specificaiton using semiregular degenerate refinement approach.
Figure 123 shows overview of the method.
This chapter discusses only the creation of the grid geometry. Mapping and coding are discussed in Chapters 345 and 345, respectively.
...
For this thesis, we define the radius of a polyhedron to be the radius of its circumscribed sphere. Thus, all points on a polyhedron have the same `radius' (Figure~\ref{fig:layers}).
...
For simplicity, we initially assume the surface refinement factor is 1:4; a generalization to other refinement factors is given later in Chapter~\ref{sec:grid:factors}.
