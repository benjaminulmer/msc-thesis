...
Given these goals and scope, we can refine the goal of this thesis: develop methods Earth-centric 3D DGGS's using semiregular refinement to support large ranges of altitude, equal volume cells, efficient operations, and interoperability with 2D DGGS's.
...
It also allows for an additional property, target aspect ratio of cells, to be attained.
...
\begin{enumerate}
	...
	Furthermore, clearly defined parameters allow for target aspect ratio of cells and---like the SDOG modifications---a balanced trade-off between volume preservation and cell compactness.
...
The grid extension method along with its mappings and use case results have been submitted to the journal \textit{ISPRS International Journal of Geo-Information}.
...
Chapter~\ref{chap:sdog} introduces SDOG and provides an analysis for the number of cells in SDOG at a given resolution.
...
Chapter~\ref{chap:usecases} showcases three sample use cases for the grid extensions method along with the 3D DGGS's used for said use cases.
