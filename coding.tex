...
A final interesting note is that runtime for direct decoding of the modified grids grows until about resolution four, where it then levels off to a constant.
We are not entirely sure why this is, though we suspect it relates to the number of cells on the border of the octant.
Lower resolutions have more cells on the border, and we suspect the inverse mapping functions may evaluate more quickly for certain values of $\hat{\varphi}$ and $\hat{r}$---specifically $\hat{\varphi} = 0,1$ and $\hat{r} = 0$.
However, this is only a conjecture; more rigorous benchmarking and evaluation would be needed to justify this claim.
