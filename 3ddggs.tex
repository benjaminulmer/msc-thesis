...
We argue for the Earth-centric approach, proposing semiregular refinement---the primary tool used in the remainder of this thesis---as a strategy to lessen the disadvantages of this type of grid.
...
Similar radial extensions are also possible for many of the DGG's and DGGS's explored in Chapter~24525.
...
In this thesis, we propose the term \textit{semiregular refinement} to refer to this class of methods, due to the semiregular grids they produce.
...
Hence, we call this semiregular refinement.
...
In general, we define a semiregular refinement to be one in which one or more splitting geometries (curve in 2D and surface in 3D) are stopped by another splitting geometry in the direction toward a singularity where, if not stopped, would otherwise intersect said singularity.
Such a definition describes the methods used in QTM, SDOG, Sphere Shell Space 3D Grid, the great circle arc QTM extension, and those used in Chapter~6236 of this thesis.



Allows for more uniform cells at cost of degenerate grid connectivity and slightly more complicated (degenerate) grid connectivity.
