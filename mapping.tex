...
Referring back to \cref{fig:sdog-shells} for SDOG and comparing it to \cref{fig:prismatoid-shells} for the extension method, radial splits have the same effect and structure.
...
Looking at \cref{fig:sdog-shells} and \cref{fig:prismatoid-shells}, we see these are simply the shell number and shell number plus one for the upper and lower, respectively.
The shell that contains $\hat{r}$ is given by $s = \lfloor \log_{0.5} \hat{r} \rfloor$.
Therefore, the actual radii in both physical and grid space are obtained by raising $\alpha$ and $0.5$ to these powers.
...
\end{equation}
%
For the interpolation function, how $d$ is interpolated between $\ell_p$ and $u_p$ affects volume preservation.
...
Note that the output of both \cref{eq:radialInv,eq:radialForw} are also normalized and must be multiplied by $R_\mathrm{max}$ for the true radii.
...
Just as with the radial mappings, our goal is to define a function $M_\varphi(\varphi)$ that maps a physical latitude to one in the grid domain. Likewise, we consider latitude to be normalized with $\hat{\varphi} = \pm (2\varphi) / \pi$, where the sign is set according to the sign of the latitude.
...
Additionally, for the physical domain values, these are the sine of angles as opposed to angles themselves since \cref{eq:latVol} operates on the sine of angles.
