...
As we will explore in Chapter~\ref{chap:coding}, the regularity of midpoint refinement in SDOG allows for efficient encoding and decoding algorithms---this is not the case for our modifications.
...
Note that the output of both Equations~(\ref{eq:radialInv}) and (\ref{eq:radialForw}) are also normalized and must be multiplied by $R_\mathrm{max}$ for the true radii.
...
Just as with the radial mappings, the output of Equations~(\ref{eq:latInv}) and (\ref{eq:latForw}) are normalized and must be multiplied by $\pm \pi / 2$ for the true latitudes.
