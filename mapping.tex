...
\begin{figure}[ht!]
	...
	\caption[Overview of mapping for semiregular degenerate refinement grids]{
		Overview of mapping between grid and physical domains for a semiregular degenerate refinement grid.
...
Just as with the radial mappings, our goal is to define a function $M_\varphi(\varphi)$ that maps a physical latitude to one in the grid domain. Likewise, we consider latitude to be normalized with $\hat{\varphi} = \pm (2\varphi) / \pi$, where the sign is equal to the sign of the latitude.
