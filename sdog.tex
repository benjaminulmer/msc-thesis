...
The first approach we use to accomplish this is modifying an existing 3D DGGS in order to improve its volume-preserving properties.
...
Due to the semiregular degenerate nature of SDOG refinement, calculating the number of cells in the grid is more complicated than a simple exponential formulation.
...
%
\begin{equation*}
N(k) = \frac{1}{21} \left( 7 \cdot 2^{k} + 8^{k+1} + 6 \right) - 2^{k}, \quad\text{and}
\end{equation*}
%
\begin{equation*}
T(k) = S(k) + L(k) + N(k) = \frac{1}{21} \left( 7 \cdot 2^{k} + 8^{k+1} + 6 \right).
\end{equation*}
%
