...
Most of the content of this chapter is taken from the article "Toward Volume Preserving Spheroid-Degenerated Octree Grid," authored by Benjamin Ulmer and Faramarz Samavati, which appeared in the journal \textit{Geoinformatica}~\cite{ulmer2020toward}.
...
From this we say
%
\begin{equation*} \label{eq:lg-recursive}
...
Thus
%
\begin{equation*} \label{eq:ng-recursive}
...
%
\begin{equation} \label{eq:lg-closed}
L(k) = 2^{k} - 1,
\end{equation}
%
\begin{equation} \label{eq:ng-closed}
...
\begin{figure}[ht!]
	\includegraphics[width=0.5\textwidth]{sdog-functions.pdf}
	\caption[Title]{
...
Conventional SDOG used midpoints (i.e. $\alpha = \frac{1}{2}$) for each spherical coordinate during refinement, regardless of cell type.
...
Referring to Eq.~(\ref{eq:volume}), let $p \in (0,1)$ be the percentage we wish for the lower range to have, then
%
\begin{equation*}
...
From Eq.~(\ref{eq:longVol}), it is clear a longitudinal splitting surface at the midpoint will always split a cell exactly in half, and therefore they should not be changed.
...
Ideal splitting surfaces to maximize volume preservation are achieved using Eqs.~(\ref{eq:radVol}) and (\ref{eq:latVol}); all that is required is to determine the ideal value of $p$ for the different splitting surfaces and cell types.
...
Therefore, we require that the volume on each side of the regular splitting surfaces be equal, or in other words, we set $p = \frac{1}{2}$ for these splitting surfaces.
...
Zone $n$ has a volume equal to $\left( 1 - p \right)^{n} p$ that of the initial octant, then setting the ratio between zone $n+1$ and $n$ to be equal to $\frac{1}{4}$ gives us $p = \frac{3}{4}$.
...
In this grid, all NG cells at the same level of refinement have equal volume; this dramatically improves the volume preservation, as only cells that extend to one of the singularities (SG and LG cells) will have a different volume than the other cells in the grid.
...
Our first balanced scheme then is to leave the regular splitting surfaces at the midpoints and only modify the latitudinal splitting surfaces for SG and LG cells.
...
Eqs.~(\ref{eq:radVol}) and (\ref{eq:latVol}) give ideal placements for volume preservation as compared to the conventional midpoint functions which result in better compactness.
...
%%%%%%%%
where $1 \leq t_r \leq 3$ blends between compactness at $t_r = 1$ (equivalent to midpoint) and volume preservation at $t_r=3$ (equivalent to Eq.~(\ref{eq:radVol})).
For the latitudinal splitting surfaces, we propose
%%%%%%%
where $1 \leq t_\phi \leq \infty$ blends between volume preservation at $t_\phi = 1$ (equivalent to Eq.~(\ref{eq:latVol})) and compactness at $t_\phi = \infty$ (equivalent to midpoint). We found values of $t_r = 2$ and $t_\phi = 1.5$ to offer a relatively balanced tradeoff between the two properties, with the resulting grid shown in Figure~\ref{fig:blend-volume}.
We refere to this method as the \textit{balanced} one.
