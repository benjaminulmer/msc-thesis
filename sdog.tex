...
Second, the use of spherical coordinates in SDOG refinement allows for efficient coding operations, which we discuss in Chapter~\ref{chap:coding}.
...
These midpoints create splitting surfaces used to divide parent cells into smaller children cells, shown in Figure~\ref{fig:sdog-refinement}.
...
Semiregular degenerate refinement makes this formulation more complicated.
...
Combined with the volume of cells, this allows us to measure the compactness of cells, which we use in Section~\ref{chap:4:results} to help evaluate the consequences of our modifications.
...
As a baseline, the distribution of cell volumes in a conventional SDOG grid is visualized in Figure~\ref{fig:sdog-volume}; the largest cell has approximately 8.9 times the volume of the smallest cell.
...
Let $c_{s}$ be the location of the splitting surface, where $c$ is one of $\left\lbrace \varphi, \lambda, r \right\rbrace$ (Figure~\ref{fig:functions}).
...
Each spherical coordinate has a different effect on volume (Equation~\ref{eq:volume}); therefore, this function is different for each.
...
Zone $n$ has a volume equal to $\left( 1 - p \right)^{n} p$ that of the initial octant, so setting the ratio between zone $n+1$ and $n$ to be equal to $1/4$ gives us $p = 3/4$.
...
Only cells that extend to one of the singularities (i.e. SG and LG cells) will have a smaller volume than the other cells in the grid---by a factor of approximately 2.6 and x.x, respectively.
...
Comparing this to conventional SDOG (refer back to Figure~\ref{fig:sdog-volume}), the two grids are nearly identical, with only slight variation in the placement of latitude splitting surfaces.
...
There are endless possibilities for these functions; however, as will be required for coding in Chapter~\ref{chap:mapping}, they should be easily invertible.
...
\end{equation}
%
where $1 \leq h \leq \infty$ blends between volume preservation at $h = 1$ (equivalent to Equation~(\ref{eq:latVol})) and compactness as $h \rightarrow \infty$ (equivalent to midpoint).
We found values of $t = 2$ and $h = 1.45$ to offer a relatively balanced tradeoff between the two properties, with the resulting grid shown in Figure~\ref{fig:balanced-volume}.
